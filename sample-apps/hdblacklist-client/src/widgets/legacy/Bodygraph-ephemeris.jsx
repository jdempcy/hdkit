...
import sparkles from "../assets/sparkle.gif";

class Bodygraph extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isNewChangeAppearing: false,
      previousMatch: false
    };
  } // End constructor

  updateBodygraphActivationsForCurrentTime(ephemeris) {
 ...
 drawBodygraph(match) {
  ...
  let pMatch = null;;
  if (!this.state.previousMatch) { 
    this.setState({previousMatch: match}); 
    pMatch = match;
  } else {
    pMatch = this.state.previousMatch;
  }
  
  if ((match.Sun.t != pMatch.Sun.t) || 
      (match.NorthNode.t != pMatch.NorthNode.t) ||  
      (match.Moon.c != pMatch.Moon.c) ||
      (match.Mercury.c != pMatch.Mercury.c) ||
      (match.Venus.c != pMatch.Venus.c) ||
      (match.Mars.c != pMatch.Mars.c) ||
      (match.Jupiter.c != pMatch.Jupiter.c) ||
      (match.Saturn.c != pMatch.Saturn.c) ||
      (match.Uranus.c != pMatch.Uranus.c) ||
      (match.Neptune.c != pMatch.Neptune.c) ||
      (match.Pluto.c != pMatch.Pluto.c)) {
    // There's been a change!
    this.state.isNewChangeAppearing = true;
    setTimeout(() => {this.state.isNewChangeAppearing = false}, 1500);
  }
  // console.log(match);
  // e.g. ...
  ...
  let textBgOutlineHeadGates = document.querySelectorAll(
    "#GateTextBg64 path:first-child, #GateTextBg61 path:first-child, #GateTextBg63 path:first-child"
  );
  ...
  for (let i = 1; i <= 64; i++) {
    ...
    let textBg = document.querySelector(
      `#GateTextBg${i} path:first-child, #GateTextBg${i} circle:first-child`
    );
  ...
  // Head-Ajna channels
  if (
    (activations.includes(64) && activations.includes(47)) ||
  ...
  if (
    (activations.includes(17) && activations.includes(62)) ||
  ...
  // Throat-G channels
  if (
    (activations.includes(20) && activations.includes(10)) ||
  ...
  if (
    (activations.includes(12) && activations.includes(22)) ||
  ...
  if (
    (activations.includes(20) && activations.includes(57)) ||
  ...
  if (
    (activations.includes(50) && activations.includes(27)) ||
  ...
  // Sacral-G channels
  if (
    (activations.includes(34) && activations.includes(10)) ||
  ...
  if (
    (activations.includes(53) && activations.includes(42)) ||
  ...
  if (
    (activations.includes(54) && activations.includes(32)) ||
  ...
  if (
    (activations.includes(19) && activations.includes(49)) ||
  ...
  componentDidMount() {
    ...
    for (let i = 1; i <= 64; i++) {
      document.querySelector(`#GateText${i}`).setAttribute('font-size', '18');
      document.querySelectorAll(`#GateTextBg${i} path`).forEach((el) => { el.setAttribute('transform-origin', '50% 50%')});
      document.querySelector(`#GateText${i}`).setAttribute('font-weight', '600');    
    }

    let year = DateTime.now().toFormat("yyyy");
    this.drawBodygraphActivationsForCurrentTime(this.props.ephemeris);
    this.interval = setInterval(() => {
      this.updateBodygraphActivationsForCurrentTime(this.props.ephemeris);
    }, 3000);
  }
    ...
    return(
      ...
      <link href="https://fonts.googleapis.com/css?family=Open+Sans:300,400,600" rel="stylesheet" />
        <BodygraphSVG 
                  style={{height: 'auto'}}
                  />
        <Fade in={this.state.isNewChangeAppearing}><img
          src={sparkles}
          id="sparkles"
          alt="Sparkles"
